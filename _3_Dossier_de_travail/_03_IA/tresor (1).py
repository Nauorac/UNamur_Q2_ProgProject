...
def Ai(teams,equipe):
    ...
    #recupere la position de mon alpha et alpha ennemis
    position_alpha=[]
    ...
    for cle in teams:
        # Check if it's alpha of the team in argument
        if teams[cle][0]== equipe and teams[cle][1]== "alpha":
            position_alpha.append(cle[0])
            position_alpha.append(cle[1])
        # Check if it's an opposite alpha of the teams in argument
    ...
    # Ordres Alpha
    # ------------
    dic_loup=in_range(teams,1,position_alpha) #dictionnaire des loups amis et ennemis
    for cle in dic_loup:
    ...
    if (teams[(position_alpha[0],position_alpha[1])][2]<100):
        for cle in teams:
            if(teams[(cle[0],cle[1])][0]==0 and distance(position_alpha[0],position_alpha[1],cle[0],cle[1])==1 ):
            # Verifier si mon alpha est sur une ressource et il a un manque d'energie
                ordre_alpha = ""+str(position_alpha[0]) + "-" + str(
                position_alpha[1])+":<" + str(cle[0]) + "-" + str(cle[1])+""
    else:
    # Traiter le cas ou le loup est n'est pas a cote une ressource ou il assez d'energie
        x=""
        y=""
        if (ennemi_alpha==[]):
    # Verifier si on a pas de loups ennemis a cote de nous
            ...
            if position_alpha_ennemi[1]> position_alpha[1]:
                ...
                y= position_alpha[1]
            ordre_alpha = "" + \
                str(position_alpha[0])+"-" + str(position_alpha[1]
                                                 ) + ":@" + str(x) + "-" + str(y)+""
        else:
        # Le cas ou on des les loups ennemis qui nous entourent
            for i in ennemi_alpha:
                if teams[(i[0],i[1])][1]=="alpha":
                    ordre_alpha = ""+str(
                        position_alpha[0]) + "-" + str(position_alpha[1])+":*"+str(i[0])+"-" + str(i[1])+""
            z= random.choice(ennemi_alpha)
            ordre_alpha = ""+str(position_alpha[0]) + "-" + str(
                position_alpha[1]) + ":*" + str(z[0])+"-" + str(z[1])+""
    # ------------
    # Ordres Omega
    # ------------
    # Le cas ou notre loup omega n'a pas assez d'energie pour pacifier
    if (teams[(position_omega[0],position_omega[1])][2]<40):
    ...
    # Le cas ou notre loup omega a des ennemis qui l'entourent
        if ennemi_omega!=[]:
            if position_alpha_ennemi in ennemi_omega:
                ordre_omega = ""+str(position_omega[0]) + "-" + str(position_omega[1])+":*"+str(
                    position_alpha_ennemi[0])+"-" + str(position_alpha_ennemi[1])+""
            else:
                position=random.choice(ennemi_omega)
                ordre_omega = ""+str(position_omega[0]) + "-" + str(
                    position_omega[1])+":*"+str(position[0])+"-" + str(position[1])+""
        else:
    # Le cas ou il n'a pas de loups ennemis a cote de notre loup omega
            ...
            for cle in teams:
                if teams[cle][0]==0 :
                    ...
                    if distance<dis:
                        proche=cle
            ordre_omega = ""+str(position_alpha[0]) + "-" + str(
                position_alpha[1])+":*"+str(proche[0])+"-" + str(proche[1])+""
    else:
        ennemis_rayon=in_range(teams,6,position_omega)
        if len(ennemis_rayon)>3:
            ordre_omega = ""+ \
                str(position_omega[0]) + "-" + \
                str(position_omega[1]) + ":pacify"
        else:
            if ennemi_omega!=[]:
                if [ennemi_alpha[0],ennemi_alpha[1]] in ennemi_omega:
                    ordre_omega = ""+str(position_alpha[0]) + "-" + str(position_alpha[1])+":*"+str(
                        ennemi_alpha[0])+"-" + str(ennemi_omega[1])+""
                else:
                    position=random.choice(ennemi_omega)
                    ordre_omega = ""+str(position_alpha[0]) + "-" + str(
                        position_alpha[1])+":*"+str(position[0])+"-" + str(position[1])+""
            else:
